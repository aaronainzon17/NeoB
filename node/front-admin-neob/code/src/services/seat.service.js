...
class SeatService {
  ...
  { var estados = new Map();
    ...
    try{
      const aux = await axios.post(http.URL_RESERVA_ST,dataaux,AuthHeaderService.generalauthHeader());
      const ocupados=aux.data.ocupados;
      const escaneados=aux.data.escaneados;
      ...
      if(resume.status === 200){//todo ok
        ...
        for (const fila in resume.data.filas){
          if(resume.data.filas[fila].estadoasiento === "L"){
            estado = "libre";
          }else if(resume.data.filas[fila].estadoasiento === "O"){
            estado = "ocupado";
          }else if(resume.data.filas[fila].estadoasiento === "S"){
            estado = "sucio";
          }else if(resume.data.filas[fila].estadoasiento === "N"){
            estado = "no disponible";
          }else if(resume.data.filas[fila].estadoasiento === "E"){
            estado = "escaneado";
          }
